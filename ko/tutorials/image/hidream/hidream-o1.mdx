...
description: "이 가이드는 ComfyUI 네이티브 HiDream-O1-Image text-to-image 및 이미지 편집 워크플로우를 안내합니다"
...
HiDream-O1-Image는 2026년 5월 8일 HiDream-ai가 오픈소스로 공개한 통합 이미지 생성 기반 모델입니다. 픽셀 수준의 통합 트랜스포머(UiT)를 기반으로 구축되었으며, 외부 VAE나 분리된 텍스트 인코더 없이 작동하며, 최대 2,048×2,048 해상도에서 **text-to-image**, **지시어 기반 이미지 편집**, **주제 중심 개인화**, **스토리보드 생성**을 지원합니다.
...
- **text-to-image 모드:** **"이미지 편집으로 전환"** 토글을 **꺼짐**으로 설정하세요(기본값). 샘플러는 직접 텍스트 프롬프트를 사용합니다.
...
- **text-to-image 모드:** **"이미지 편집으로 전환"** 토글을 **꺼짐**으로 설정하세요(기본값)
