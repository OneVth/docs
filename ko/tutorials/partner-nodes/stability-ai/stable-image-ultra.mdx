...
description: "이 기사에서는 Stability AI Stable Image Ultra 파트너 노드의 text-to-image 및 image-to-image 기능을 ComfyUI에서 사용하는 방법을 소개합니다."
...
이 가이드에서는 이 노드를 사용해 text-to-image와 image-to-image 생성 워크플로우를 설정하는 방법을 보여드립니다.

<ReqHint/>
<UpdateReminder/>

## Stability AI Stable Image Ultra Text-to-Image 워크플로우

### 1. 워크플로우 파일 다운로드

워크플로우 정보는 아래 이미지의 메타데이터에 포함되어 있습니다. 해당 워크플로우를 다운로드하여 ComfyUI로 드래그해 로드해 주세요.

![Stability AI Stable Image Ultra text-to-image 워크플로우](https://raw.githubusercontent.com/Comfy-Org/example_workflows/main/api_nodes/stability_ai/stable_image_ultra_t2i.png)

### 2. 워크플로우를 단계별로 완료하세요

![Stability AI Stable Image Ultra text-to-image 단계 안내](/images/tutorial/api_nodes/stability_ai/stable_image_ultra_t2i_step_guide.jpg)

이미지의 번호가 매겨진 단계를 따라 기본적인 text-to-image 워크플로우를 완료할 수 있습니다:
...
- 현재 `Load Image` 노드는 "Bypass" 모드입니다. 이를 활성화하려면 단계 안내를 참고해 해당 노드를 마우스 오른쪽 버튼으로 클릭한 후 "Mode"를 "Always"로 설정해 입력을 활성화하면 image-to-image 모드로 전환됩니다.

## Stability AI Stable Image Ultra Image-to-Image 워크플로우

### 1. 워크플로우 파일 다운로드

워크플로우 정보는 아래 이미지의 메타데이터에 포함되어 있습니다. 해당 워크플로우를 다운로드하여 ComfyUI로 드래그해 로드해 주세요.

![Stability Stable Image Ultra image-to-image 워크플로우](https://raw.githubusercontent.com/Comfy-Org/example_workflows/main/api_nodes/stability_ai/i2i/stable_image_ultra_i2i.png)

아래 이미지를 다운로드해 입력으로 사용하세요
![Stability Stable Image Ultra image-to-image 워크플로우 입력 이미지](https://raw.githubusercontent.com/Comfy-Org/example_workflows/main/api_nodes/stability_ai/i2i/input.png)

### 2. 워크플로우를 단계별로 완료하세요

![Stability Stable Image Ultra image-to-image 단계 안내](/images/tutorial/api_nodes/stability_ai/stable_image_ultra_i2i_step_guide.jpg)

이미지의 번호가 매겨진 단계를 따라 image-to-image 워크플로우를 완료할 수 있습니다:
...
3. (선택사항) `image_denoise` 매개변수(범위 0.0-1.0)를 조정해 원본 이미지의 수정 정도를 제어하세요:
   ...
   - 1.0에 가까울수록 생성된 이미지는 순수한 text-to-image 생성과 더 비슷해집니다.
...
**이미지 노이즈 제거**: 이 매개변수는 생성 과정에서 원본 이미지의 특징을 얼마나 유지할지 결정하며, image-to-image 모드에서 가장 중요한 조정 매개변수입니다. 아래 이미지는 서로 다른 노이즈 제거 강도의 효과를 보여줍니다.

![Stability Stable Image Ultra image-to-image 노이즈 제거 설명](/images/tutorial/api_nodes/stability_ai/i2i_image_denoise.jpg)

- **참조 이미지 선택**: 명확한 피사체와 좋은 구성을 가진 이미지를 선택하면 보통 더 나은 결과를 얻을 수 있습니다.
- **프롬프트 팁**: image-to-image 모드에서는 프롬프트가 이미지에 이미 존재하는 모든 요소를 설명하기보다는 변경하거나 강조하고자 하는 부분에 집중해야 합니다.