---
title: "ComfyUI Flux.1 Text-to-Image 워크플로우 예시"
description: "이 가이드는 Flux.1 모델에 대한 간략한 소개와 함께, 전체 버전 및 FP8 체크포인트 버전을 포함한 예시를 통해 Flux.1 모델을 이용한 text-to-image 생성 방법을 안내합니다."
...
Flux는 120억 파라미터를 가진 오픈소스 text-to-image 생성 모델 중 가장 큰 모델 중 하나로, 원본 파일 크기는 약 23GB입니다. 이 모델은 전 스테이블 디퓨전 팀원들이 설립한 [Black Forest Labs](https://blackforestlabs.ai/)에서 개발되었습니다.
...
이 예시에서는 Flux.1 Dev와 Flux.1 Schnell 버전을 모두 사용해 text-to-image 예시를 소개하며, 여기에는 전체 버전 모델과 간소화된 FP8 체크포인트 버전이 포함됩니다.
...
## Flux.1 FP8 체크포인트 버전 Text-to-Image 예시
